...
/**
 * playStampChime — 金属＋木のスタンプ音 (ポンッ！).
 * Short metallic hit (1200Hz) + wooden thud (800Hz), ~0.25s.
 */
export function playStampChime(): void {
  if (muted) return;
  const ac = getContext();
  if (!ac) return;
  try {
    if (ac.state === "suspended") ac.resume().catch(() => undefined);
    // Metal ping: 1200Hz, very short
    playPartial(ac, 1200, 0.14, 0,    0.18);
    // Wooden body: 800Hz, slightly delayed, longer
    playPartial(ac, 800,  0.10, 0.02, 0.25);
    // Resonance: 400Hz, quiet
    playPartial(ac, 400,  0.04, 0.04, 0.20);
  } catch { /* silent fail */ }
}
...
export const STAMP_CHIME_FREQUENCIES = [1200, 800, 400] as const;
